...
if __name__ == "__main__":
    # The Focus layer for YOLOv5 typically uses a 3x3 convolution
    p1 = Focus(in_channels=3, out_channels=64, kernel_size=3, stride=1, padding=1)
    p2 = ConvBNSiLU(in_channels=64, out_channels=128, kernel_size=3, stride=2, padding=1)

    c3_1 = C3(in_channels=128, out_channels=128, n=3)
    p3 = ConvBNSiLU(in_channels=128, out_channels=256, kernel_size=3, stride=2, padding=1)
    
    c3_2 = C3(in_channels=256, out_channels=256, n=6)
    p4 = ConvBNSiLU(in_channels=256, out_channels=512, kernel_size=3, stride=2, padding=1)
    
    c3_3 = C3(in_channels=512, out_channels=512, n=9)
    p5 = ConvBNSiLU(in_channels=512, out_channels=1024, kernel_size=3, stride=2, padding=1)

    c3_4 = C3(in_channels=1024, out_channels=1024, n=3)

    sppf = SPPF(in_channels=1024, out_channels=1024, kernel_sizes=(5, 5, 5))

    # Create a dummy input tensor to simulate a 640x640 RGB image
    input_tensor = torch.randn(1, 3, 640, 640)

    # Forward pass through the Focus layer
    output_tensor = p1(input_tensor)
    output_tensor = p2(output_tensor)
    output_tensor = c3_1(output_tensor)
    output_tensor = p3(output_tensor)
    output_tensor = c3_2(output_tensor)
    output_tensor = p4(output_tensor)
    output_tensor = c3_3(output_tensor)
    output_tensor = p5(output_tensor)
    output_tensor = c3_4(output_tensor)
    output_tensor = sppf(output_tensor)
